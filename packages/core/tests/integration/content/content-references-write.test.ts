...
describeEachDialect("content create with a `references` key", (dialect) => {
	...
	it("writes edges from a non-default-locale entry that its sibling locale reads back", async () => {
		ctx = await setupForDialect(dialect);
		try {
			const { relation } = await setupPostsWithRelation(ctx.db);
			...
			// The child exists only in `en`; the parent is edited in `fr`. Edges are
			// keyed by translation_group, so neither side being in a different locale
			// makes the reference any less real.
			const child = await handleContentCreate(ctx.db, "posts", { data: { title: "Child (en)" } });
			const parentEn = await handleContentCreate(ctx.db, "posts", { data: { title: "Parent" } });
			expect(child.success && parentEn.success).toBe(true);
			if (!child.success || !parentEn.success) return;

			const parentFr = await handleContentCreate(ctx.db, "posts", {
				data: { title: "Parent (fr)" },
				locale: "fr",
				translationOf: parentEn.data.item.id,
				references: { [relation.translationGroup]: [child.data.item.id] },
			});
			expect(parentFr.success).toBe(true);
			if (!parentFr.success) return;

			// Written from the `fr` row, visible from the `en` row: same group, same edges.
			const fromEn = await handleContentGet(ctx.db, "posts", parentEn.data.item.id, undefined, {
				includeDrafts: true,
			});
			expect(fromEn.success).toBe(true);
			if (!fromEn.success) return;
			expect(
				fromEn.data.item.references?.[relation.translationGroup]?.children.map((c) => c.id),
			).toEqual([child.data.item.id]);

			// Read from `fr`, the child has no `fr` variant, so it resolves to the
			// `en` row and says so rather than pretending to be French.
			const fromFr = await handleContentGet(ctx.db, "posts", parentFr.data.item.id, "fr", {
				includeDrafts: true,
			});
			expect(fromFr.success).toBe(true);
			if (!fromFr.success) return;
			const frChildren = fromFr.data.item.references?.[relation.translationGroup]?.children;
			expect(frChildren?.map((c) => c.id)).toEqual([child.data.item.id]);
			expect(frChildren?.[0]?.locale).toBe("en");
		} finally {
			await teardownForDialect(ctx);
		}
	});
